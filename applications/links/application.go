...
import (
	application_layers "github.com/steve-care-software/datastencil/applications/links/layers"
...
func (app *application) execute(input []byte, link links.Link, context commands.Commands) (commands.Commands, error) {
	...
	for _, oneElement := range elements {
		layerHash := oneElement.Layer()
		layer, err := app.layerRepository.Retrieve(layerHash)
		if err != nil {
			return nil, err
		}
