...
type application struct {
	...
	commandsAdapter     commands.Adapter
}
...
// Compile compiles a script into commands using the provided compiler
func (app *application) Compile(compiler compilers.Compiler, script []byte) (commands.Commands, []byte, error) {
	output := []byte{}
	remaining := script
	...
	instructions, err := app.commandsAdapter.ToCommands(output)
	if err != nil {
		return nil, nil, err
	}
...
func (app *application) Program(commands commands.Commands) (programs.Program, error) {
	return nil, nil
}
