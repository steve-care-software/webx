...
func TestModule_newGrammarValue_Success(t *testing.T) {
	...
	name := "roger"
	output, remaining, err := applications.NewApplication(NewApplication(bitrateForTests, basePathForTests, delimiterForTests, extensionForTests)).ParseThenInterpret(map[string]interface{}{
		valueToHashStringForTests("myName"): name,
	}, []byte(script))
	...
	if ins, ok := output[valueToHashStringForTests("myValue")].(values.Value); ok {
		retName := ins.Name()
		if retName != name {
