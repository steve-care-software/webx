...
func (app *mainIns) Fetch() *fyne.Container {
	...
	// Create a new Entry (TextField), multi-line:
	entry := widget.NewMultiLineEntry()
	entry.SetText(string(grammarInput))
	entry.Resize(fyne.NewSize(800, 600))

	// Create a container with a max layout to fill 100% width and height
	textFieldContainer := container.New(layout.NewStackLayout(), entry)
